...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, act_dim, num_layer=2, gamma=0.99, hidden_sizes=[512, 256]):
        super(ActorCritic, self).__init__()
        self.state_dim = state_dim      # 400
        self.act_dim = act_dim          # 251
        self.gamma = gamma              # 0.99
        self.num_layer = num_layer

        self.lstm   = nn.LSTM(input_size=state_dim, hidden_size=hidden_sizes[0], num_layers=self.num_layer, batch_first=True)
        self.lstm_hiddensize = hidden_sizes[0]
    ...
    def forward(self, inputs):
        ...
        init_h = zeros_var_cuda([self.num_layer, len(state), self.lstm_hiddensize])
        init_c = zeros_var_cuda([self.num_layer, len(state), self.lstm_hiddensize])
        input_l = state.unsqueeze(1)
        x = self.lstm(input_l, (init_h, init_c))[0].squeeze(1)
...
def train(args):
    ...
    model = ActorCritic(env.state_dim, env.act_dim, num_layer=args.lstm_layer, gamma=args.gamma, hidden_sizes=args.hidden).to(args.device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while dataloader.has_next():
            batch_uids = dataloader.get_batch() # 32 uids
...
def main():
    ...
    parser.add_argument('--lstm_layer', type=int, default=2, help='number of BiLSTM layers')
