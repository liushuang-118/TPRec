...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        while dataloader.has_next():
            ...
            # batch_idxs = torch.from_numpy(batch_idxs).cuda()

            # Train model.
            optimizer.zero_grad()
            train_loss = model(batch_idxs)  # 调用 Forward.
            train_loss.backward()
...
def extract_embeddings(args):
    ...
    for i in range(CLUSNUM):
        embeds[PURCHASE[i]] = (
            state_dict['purchase_' + str(i)].cpu().data.numpy()[0],
            state_dict['purchase_' + str(i) + '_bias.weight'].cpu().data.numpy()
        )
        embeds[MENTION[i]] = (
            state_dict['mention_' + str(i)].cpu().data.numpy()[0],
            state_dict['mention_' + str(i) + '_bias.weight'].cpu().data.numpy()
        )
        embeds[DESCRIBED_AS[i]] = (
            state_dict['described_as_' + str(i)].cpu().data.numpy()[0],
            state_dict['described_as_' + str(i) + '_bias.weight'].cpu().data.numpy()
        )
