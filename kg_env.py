...
class KGState(object):
    ...
    # State defined in PGPR paper .
    # Since K-hop maximium is 3.
...
def extend_pattern(pattern, pos):
    extend_times = len(pattern[pos])
    bf_pattern = [pattern.copy() for i in range(extend_times)]
    for time_clus in range(extend_times):
        bf_pattern[time_clus][pos] = pattern[pos][time_clus]
    return bf_pattern

def second_e_pattern(pattern, first_pattern, pos):
    extend_times = len(pattern[pos])
    af_pattern = []
    for item in first_pattern:
        bf_pattern = [item.copy() for i in range(extend_times)]
        for time_clus in range(extend_times):
            bf_pattern[time_clus][pos] = pattern[pos][time_clus]
        af_pattern = af_pattern + bf_pattern
    return af_pattern

class BatchKGEnvironment(object):
    def __init__(self, dataset_str, max_acts, max_path_len=3, state_history=1, mode='train'):
        ...
        u_p_scores = np.dot(self.embeds[USER] + self.embeds[PURCHASE[0]][0], self.embeds[PRODUCT].T)
        # origin code
        ...
        for pattern_id in [1, 11, 12, 13, 14, 15, 16, 17, 18]:
            ...
            pattern_num = pattern.copy()
            count = 0
            for pos in range(len(pattern_num)):
                if type(pattern_num[pos]) is not type('stringType'):
                    count = count + 1
                    if count == 1:
        ...
        weight_usage = mode
        self.clus_weight = load_clus_weight(dataset_str, usage=weight_usage)
    ...
    def _get_actions(self, path, done):
        ...
        visited_nodes = set([(v[1], v[2]) for v in path]) # [('self_loop', 'user', 1681)]
        for r in relations_nodes:

            next_node_type = KG_RELATION[curr_node_type][r]
            next_node_ids = relations_nodes[r]
            next_node_ids = [n for n in next_node_ids if (next_node_type, n) not in visited_nodes]  # filter visited
...
# ============================================================================
        ...
        user_embed = self.embeds[USER][path[0][-1]]  # 起始embed
        scores = []
        ff = 1
        ...
        for r, next_node_id in candidate_acts:
            ...
            if next_node_type == USER:
                ...
                src_embed = user_embed + purchase_emd
            elif next_node_type == WORD:
                src_embed = user_embed + mention_emd
            else:  # BRAND, CATEGORY, RELATED_PRODUCT
                src_embed = user_embed + purchase_emd + self.embeds[r][0]
    ...
    def _get_reward(self, path):
        ...
        if curr_node_type == PRODUCT:
            ...
            u_vec = self.embeds[USER][uid] + purchase_emd
            p_vec = self.embeds[PRODUCT][curr_node_id]
            gg = np.dot(u_vec, p_vec)
            yy = self.u_p_scales[uid]
    ...
    def reset(self, uids=None):
        ...
        # clus_weight = load_clus_weight(dataset)
        # self._batch_cluster = [clus_weight[uidd] for uidd in uids]
    ...
    def batch_step(self, batch_act_idx):
        ...
        for i in range(len(batch_act_idx)):
            ...
            _, curr_node_type, curr_node_id = self._batch_path[i][-1] # batch_path: ((RELATION, USER, uid), (),...,())
